// Fill out your copyright notice in the Description page of Project Settings.


#include "HasMedkitRuijterYannick.h"

#include "AIController.h"
#include "Common/InventoryComponent.h"

#include "Items/Medkit.h"

UHasMedkitRuijterYannick::UHasMedkitRuijterYannick()
{
}

bool UHasMedkitRuijterYannick::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
	auto Survivor = Cast<ASurvivorPawn>(OwnerComp.GetAIOwner()->GetPawn());
	auto ItemsInInventory = Survivor->FindComponentByClass<UInventoryComponent>()->GetInventory();
	int MedkitCount = 0;
	//count the amount of weapons we have
	for (auto Item : ItemsInInventory)
	{
		if (Item == nullptr) continue;
		if (Item->GetItemType() == EItemType::Medkit) MedkitCount++;
	}
	return MedkitCount > 0;
}